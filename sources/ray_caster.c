...
/*   Updated: 2025/02/21 11:48:02 by otboumeh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	select_ray_hit(t_cub *c, t_ray *r)
{
	if (r->found_horizontal_wall_hit)
		r->horizontal_hit_distance = distance_between_points(c->p_x, \
		c->p_y, r->horizontal_wall_hit_x, r->horizontal_wall_hit_y);
	else
		r->horizontal_hit_distance = INT_MAX;
	if (r->found_vertical_wall_hit)
		r->vertical_hit_distance = distance_between_points(c->p_x, \
		c->p_y, r->vertical_wall_hit_x, r->vertical_wall_hit_y);
	else
