...
/*   Updated: 2025/02/19 18:24:00 by otboumeh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	minimap_print(int x_map, int y_map, uint32_t element, t_cub *c)
{
	...
	while (y < c->minimap_tile_size)
	{
		...
		while (x < c->minimap_tile_size)
			mlx_put_pixel(c->win_mlx3D, x++ + x_map * c->minimap_tile_size \
			+ c->minimap_start_x, y + y_map * c->minimap_tile_size \
			+ c->minimap_start_y, element);
		y++;
	}
}
	...
	while (y < c->minimap_player_size)
	{
		...
		while (x < c->minimap_player_size)
		{
			mlx_put_pixel(c->win_mlx3D,
				...
				+ c->minimap_start_x, y + c->p_y / TILE_SIZE * \
				c->minimap_tile_size + c->minimap_start_y, RED);
			x++;
		}
		y++;
	}
